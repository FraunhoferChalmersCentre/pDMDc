...
%------------- BEGIN CODE --------------
function [A_train, B_train, prediction_train, X_states_tensor, U, data_metabolomics] = pDMDcMetabotyping(data_metabolomics, input_meal, r)
%% Normalization
...
data = data_metabolomics;%nprocess(data_metabolomics,[0 0 0 0],[0 0 1 0], [], [], 1);
% for i = 1:nr_diets
%     for j = 1:79
...
stacked_input = [];
for d = 1:nr_diets
    stacked_input = [stacked_input, [input_meal(:,d) zeros(size(input_meal,1),nr_time_points-2)]];
end
%% pDMDc
...
X_states_tensor = zeros(nr_individuals,r,nr_time_points,nr_diets);
...
% if i == 1
%     for ddd = 1:3
%     plot(1:nr_time_points, normalize(squeeze(data(i,:,:,ddd))),'k', 'linewidth',2); hold on
%     plot(1:nr_time_points, normalize(squeeze(prediction_train(i,:,:,ddd))))
%     xlabel('Time points')
