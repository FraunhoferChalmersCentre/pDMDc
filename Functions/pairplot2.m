function pairplot2(meas, label, group, limits, mode, group2)
...
colors = lines(length(unique(group)));
group_numbers = grp2idx(group);
group2_numbers = grp2idx(group2);
...
for i = 1:size(meas, 2)
	for j = 1:size(meas, 2)
		...
		if i==j
            ...
            markers = '.*^';
			for ind = 1:length(meas)
				plot(meas(ind, i), meas(ind, j), ...
					markers(group2_numbers(ind)), ...
                'Color', colors(group_numbers(ind),:)); hold on
